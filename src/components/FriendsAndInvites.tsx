...
export default function FriendsAndInvites() {
  ...
  const [activeTab, setActiveTab] = useState<'invite' | 'friends' | 'stats'>('invite');
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Tab Navigation */}
      <div className="flex gap-4 p-2 glass-surface rounded-[24px] w-fit">
        {[
          { id: 'invite', label: 'Invite Friends', icon: UserPlus },
          { id: 'friends', label: 'My Squad', icon: Users },
          { id: 'stats', label: 'Referral Stats', icon: TrendingUp },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            className={cn(
              "flex items-center gap-3 px-6 py-3 text-[10px] font-black uppercase italic tracking-tighter transition-all rounded-xl",
              activeTab === tab.id 
