...
import { Layers, Sparkles, Trophy, AlertCircle, RefreshCw } from 'lucide-react';
import { Button } from '../components/ui/Button';
import { Card, CardContent } from '../components/ui/Card';
import PullTab from '../components/games/PullTab';
import WinPopup from '../components/games/WinPopup';

export default function PullTabsPage() {
  ...
  const { data: ticketTypes, isLoading } = useQuery({
    queryKey: ['active-pull-tabs'],
    queryFn: async () => {
      const res = await fetch('/api/tickets/active');
      const data = await res.json();
      return data.types.filter((t: any) => t.type === 'pulltab');
    }
  });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter">Pull Tabs</h1>
          </div>
          <p className="text-slate-500 max-w-xl">
            Pull the tabs to reveal hidden symbols and win SC! Each pack contains multiple chances to win big.
          </p>
        </div>
            ...
            >
              <Card className="bg-slate-900 border-white/5 overflow-hidden relative">
                ...
                <CardContent className="p-6 relative">
                  ...
                  <Button 
                    className="w-full h-12 text-sm font-black uppercase tracking-widest bg-purple-600 hover:bg-purple-500"
                    onClick={() => purchaseMutation.mutate(ticket.id)}
                    disabled={purchaseMutation.isPending}
                  >
                    {purchaseMutation.isPending ? 'Processing...' : 'Buy Pack'}
                  </Button>
                </CardContent>
              </Card>
            </motion.div>
